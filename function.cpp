...
while(1){
  uLCD.text_width(2.5); //4X size text
    ...
    uLCD.locate(0,2);
    uLCD.color(RED);
    uLCD.printf("gesture_id"); //Default Green on black text

    uLCD.text_width(2.5); //4X size text
    uLCD.text_height(2.5);
    uLCD.locate(0,4);
    uLCD.color(RED);
    uLCD.printf("sequence"); //Default Green on black text
}
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void capture(){
  uLCD.reset();
   BSP_ACCELERO_Init();
   thread.start(ulcd_display_selected);
...
int break_condition=0;
    ...
    if(sequence_num==11){
      ...
      sprintf(buff, "gesture_id:%d sequence:%d", gesture_id,sequence_num);
